...
def _resolve_main_code(code: str):
    main_code = None
    result = db.session.execute(text("SELECT * FROM products_codes WHERE other_code = :code"), 
...
def _resolve_related_codes(main_code: str):
    related_codes = []
    result = db.session.execute(text("SELECT * FROM products_codes WHERE main_code = :main_code"), 
                                {"main_code": main_code}).mappings().fetchall()
    if result:
...
def _resolve_product_info(main_code: str):
    ...
    result = db.session.execute(
        text(
            ...
            "WHERE p.code = :code"
             ),{"code": main_code}).mappings().fetchone()
    if result:
...
def create_app(test_config=None):
    ...
    def home():
        ...
        return render_template('index.html', product=product_info, code_search=product_code, related_codes=related_codes_result['related_codes'])    
    ...
    def save_product():
        ...
        try:
            db.session.execute(
                ...
                {"code": code, "bar_code": bar_code, "description": description, "department": department, "mark": mark, "price": price}
            )
            db.session.commit()
